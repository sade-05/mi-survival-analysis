...
*This plot checks whether the model's predicted probabilities are trustworthy. Patients are grouped by their predicted risk of dying, from lowest to highest. For each group, we compare what the model predicted against what actually happened. If the model predicted a 20% chance of death for a group, did roughly 20% of them actually die?* 

*Points sitting on the diagonal line mean the model got it right. Points above the line mean the model underestimated risk, more people died than predicted. Points below the line mean the model overestimated risk, fewer people died than predicted. The size of each point shows how many patients were in that group.The zigzag pattern in the mid-range reflects small, unstable groups where low patient counts make observed death rates erratic. The smaller points on the right side of the plot reflect the fact that very few patients were predicted to be high risk,only about 15.9% of patients in this dataset died, so high-risk predictions are naturally rare.*
